...
BASE_URL = "git clone https://github.com/{}/HW1.git"
REMOVE_CMD = "rm -rf {}"
SEARCH_PATH = "./{}/{}"
...
def fetch_from_github(usernames, assignment, files):
    ...
    for username in usernames:
        repository_url = BASE_URL.format(username)
        os.system(repository_url)
        print('fetch success!')
        for f in files:
            exists = os.path.isfile(SEARCH_PATH.format(assignment, f))
            if not exists:
        ...
        for i in range(len(user_score) // 3):
            ...
            scores['Assignment'].append(assignment)
            scores['Tested Method'].append(tested_method)
            scores['Score'].append(score)
        clear_repository = REMOVE_CMD.format(assignment)
        os.system(clear_repository)
    score_summary = pd.DataFrame(scores)
    summary_name = '{}_scoring_summary.csv'.format(assignment)
